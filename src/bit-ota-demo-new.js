...
async function JacobToGpk(){
    ...
    txb.addInput(Buffer.from(jacobUtxoPreTx, 'hex').reverse(), 0,DEFAULT_SEQUENCE,preOutScript);

    // txb.addOutput(gpkShAddr, toGpkP2pkhValue);  // index : 0
    // txb.addOutput(gpkAddr, toGpkP2shValue);     // index : 1
    txb.addOutput(bitcoin.address.toOutputScript(gpkShAddr, network), toGpkP2pkhValue);  // index : 0
    txb.addOutput(bitcoin.address.toOutputScript(gpkAddr, network), toGpkP2shValue);     // index : 1

    let inIndex = 0

    let hash = txb.hashForSignature(0,preOutScript,1);

    // wrap the bip32 library
    const ECPair = ecpair.ECPairFactory(ecc)
    // let keyPair = ECPair.fromWIF(jacobWifPrvKey,network)
    ...
    txb.setInputScript(0,inputSig)

    let txSerialized = txb.toHex();
...
async function RedeempGpk(jacobUtxoPreTx1){
    ...
    txb.addInput(Buffer.from(jacobUtxoPreTx1, 'hex').reverse(), 0,DEFAULT_SEQUENCE,preOutScriptP2sh);
    ...
    txb.addInput(Buffer.from(jacobUtxoPreTx1, 'hex').reverse(), 1,DEFAULT_SEQUENCE,preOutScript);

    txb.addOutput(bitcoin.address.toOutputScript(jacobAddr, network), Number(toJacobP2pkhValue)+Number(toJacobP2shValue));

    // const tx = txb.buildIncomplete();

    let sigHash1 = txb.hashForSignature(0,preOutScriptP2sh,1);
    ...
    txb.setInputScript(0, redeemScriptSig);

    let sigHash = txb.hashForSignature(1,preOutScript,1);
    ...
    txb.setInputScript(1, scriptSig);

    console.log("scriptSig is :",scriptSig);
    console.log("txSerialized1 is ",txb.toHex());
    return txb.toHex();
};
